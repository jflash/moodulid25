...
class plgVmPaymentswed extends vmPSPlugin {
    ...
    function plgVmConfirmedOrder($cart, $order) {
        ...
        $VK_a['VK_RETURN'] = JROUTE::_(JURI::root() . 'plugins/vmpayment/swed/bank.php');
...
//    	return $this->processConfirmedOrderPaymentResponse(true, $cart, $order, $html, , $method->status_canceled);
    ...
    function plgVmOnShowOrderBEPayment($virtuemart_order_id, $virtuemart_payment_id) {
   	if (!$this->selectedThisByMethodId($virtuemart_payment_id)) {
    	...
    	$html .= $this->getHtmlRowBE('SWED_PAYMENT_NAME', $paymentTable->payment_name);
    	$html .= '</table>' . "\n";
    	return $html;
    }
   ...
   function plgVmOnPaymentNotification(&$html) {
    ...
    function plgVmOnPaymentResponseReceived(&$html) {
        ...
        $html = '<table>' . "\n";
            if($sig_result==0)
                $html .= $this->getHtmlRow(SWED_BANK_ERROR," ");
            $html .= '</table>' . "\n";
            ...
            $html .= $this->_getPaymentResponseHtml($payment_data, $payment_name);
                     if(isset($cart->_dataValidated)){
                         if($order['details']['BT']->order_status!=$method->status_success){
                            $this->update_status($method->status_success,$order_id,"VMPAYMENT_SWED_PAYMENT_CONFIRMED");
                        }
                        echo JText::_("VMPAYMENT_SWED_SUCCESS_MESSAGE");
                    }
            }elseif($_REQUEST['VK_SERVICE']==1901 && $sig_result==1){// makse on tühistatud
                if($order['details']['BT']->order_status!=$method->status_canceled){
            	   $this->update_status($method->status_canceled,$order_id,"VMPAYMENT_SWED_PAYMENT_CANCELED");
                }
                echo JText::_("VMPAYMENT_SWED_FAIL_MESSAGE");
            }
            $cart->emptyCart();
    ...
    function update_status($status,$order_id,$text='VMPAYMENT_SWED_PAYMENT_CANCELED'){
        if ($order_id) {
    ...
    function _getPaymentResponseHtml($bank_data, $payment_name) {
    	...
    	$html .= $this->getHtmlRow(SWED_ORDER_NUMBER, $stamp);
    	$html .= $this->getHtmlRow(SWED_AMOUNT, $bank_data['VK_AMOUNT'] . " " . $bank_data['VK_CURR']);
    	$html .= '</table>' . "\n";
    	return $html;
    }
