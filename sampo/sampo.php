...
class plgVmPaymentsampo extends vmPSPlugin {
    // instance of class
    public static $_this = false;
    var $bank_down='sampo';
    var $bank_UPPER='SAMPO';
    ...
    protected function getVmPluginCreateTableSQL() {
	   return $this->createTableSQL('Payment SAMPO table');
    }
    ...
    function plgVmConfirmedOrder($cart, $order) {
    	...
    	$html = '<table>' . "\n";
    	$html .= $this->getHtmlRow('SAMPO_PAYMENT_INFO', $dbValues['payment_name']);
    	...
    	$html .= $this->getHtmlRow('SAMPO_ORDER_NUMBER', $order['details']['BT']->order_number);
    	$html .= $this->getHtmlRow('SAMPO_AMOUNT', $currency->priceDisplay($order['details']['BT']->order_total));
    	$html .= '</table>' . "\n";
        $paymentCurrency = CurrencyDisplay::getInstance($method->payment_currency);
    	$totalInPaymentCurrency = round($paymentCurrency->convertCurrencyTo($method->payment_currency, $order['details']['BT']->order_total,false), 2);
        ...
        $VK_a['VK_RETURN'] = JROUTE::_(JURI::root() . 'plugins/vmpayment/sampo/bank.php');
        ...
        $html.=("</br>");
        $html.= ('<input type="submit" value="'.JText::_("VMPAYMENT_SAMPO_SUBMIT").'"/>' );
        $html.=( '</form>' );
...
//    	return $this->processConfirmedOrderPaymentResponse(true, $cart, $order, $html, , $method->status_canceled);
    ...
    function plgVmOnShowOrderBEPayment($virtuemart_order_id, $virtuemart_payment_id) {
   	if (!$this->selectedThisByMethodId($virtuemart_payment_id)) {
    	...
    	$html .= $this->getHtmlRowBE('SAMPO_PAYMENT_NAME', $paymentTable->payment_name);
    	$html .= '</table>' . "\n";
    	return $html;
    }
   ...
   function plgVmOnPaymentNotification(&$html) {
    ...
    function plgVmOnPaymentResponseReceived(&$html) {
        ...
        $html = '<table>' . "\n";
            if($sig_result==0)
                $html .= $this->getHtmlRow(SAMPO_BANK_ERROR," ");
            $html .= '</table>' . "\n";
            ...
            $html .= $this->_getPaymentResponseHtml($payment_data, $payment_name);
                     if(isset($cart->_dataValidated)){
                         if($order['details']['BT']->order_status!=$method->status_success){
                            $this->update_status($method->status_success,$order_id,"VMPAYMENT_SAMPO_PAYMENT_CONFIRMED");
                        }
                        echo JText::_("VMPAYMENT_SAMPO_SUCCESS_MESSAGE");
                    }
            }elseif($_REQUEST['VK_SERVICE']==1901 && $sig_result==1){// makse on tühistatud
                if($order['details']['BT']->order_status!=$method->status_canceled){
            	   $this->update_status($method->status_canceled,$order_id,"VMPAYMENT_SAMPO_PAYMENT_CANCELED");
                }
                echo JText::_("VMPAYMENT_SAMPO_FAIL_MESSAGE");
            }
            $cart->emptyCart();
    ...
    function update_status($status,$order_id,$text='VMPAYMENT_SAMPO_PAYMENT_CANCELED'){
        if ($order_id) {
    ...
    function _getPaymentResponseHtml($bank_data, $payment_name) {
       	$html = '<table>' . "\n";
    	$html .= $this->getHtmlRow(SAMPO_PAYMENT_NAME, $payment_name);
        $stamp=dechex($bank_data['VK_STAMP']);//payment method number
    	$html .= $this->getHtmlRow(SAMPO_ORDER_NUMBER, $stamp);
    	$html .= $this->getHtmlRow(SAMPO_AMOUNT, $bank_data['VK_AMOUNT'] . " " . $bank_data['VK_CURR']);
    	$html .= '</table>' . "\n";
    	return $html;
    }
